...
// FIX Warning: Remove unused eyre import
...
pub fn v3_price_from_sqrt(sqrt_price_x96: U256, decimals0: u8, decimals1: u8) -> Result<f64> {
    ...
    let q96_f64: f64 = (U256::one() << 96).to_f64_lossy();
    if q96_f64 == 0.0 || q96_f64 == f64::MAX {
...
pub fn f64_to_wei(amount_f64: f64, decimals: u32) -> Result<U256> {
    ...
    let wei_f64 = (amount_f64 * multiplier + f64::EPSILON).round();
    ...
    let wei_str = wei_f64.to_string();
    let wei_str_cleaned = wei_str.split('.').next().unwrap_or(&wei_str);

    U256::from_dec_str(wei_str_cleaned)
         .wrap_err_with(|| format!("Failed to parse f64 '{}' (wei string '{}', cleaned '{}', decimals {}) to U256", amount_f64, wei_str, wei_str_cleaned, decimals))
}